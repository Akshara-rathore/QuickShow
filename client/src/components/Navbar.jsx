  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      <div
        ...
        md:border border-gray-300/20 overflow-hidden transition-all
        duration-300 ${open ? 'max-md:w-full' : 'max-md:w-0'}`}
      >
        <XIcon
      ...
      <div className='flex items-center gap-8'>
        ...
        {!user ? (
          <button
            onClick={openSignIn}
            className='px-4 py-1 sm:px-7 sm:py-2 bg-white/10 border border-white/20 backdrop-blur-md text-white hover:bg-white/20 transition rounded-full font-medium cursor-pointer'
          >
            Login
          </button>
        ) : (
